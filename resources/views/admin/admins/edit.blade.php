...
@section('title', trans('keywords.edit admin'))
                        ...
                        <form action="{{ route('admin.admins.update', $admin->id) }}" method="POST"
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-12 col-md-6">
                                    ...
                                    <input value="{{ $admin->user->name }}" type="text" name="name"
                                ...
                                <div class="form-group col-12 col-md-6">
                                    ...
                                    <input value="{{ $admin->user->email }}" type="text" name="email"
                                        class="form-control" placeholder="">
                                    @error('email')
                                        <div class="alert alert-danger text-center">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group col-12 col-md-6">
                                    <label for="password">{{ trans('keywords.Password') }}&ThinSpace;<b
                                ...
                                <div class="form-group col-12 col-md-6">
                                    <label
                                        for="password_confirmation">{{ trans('keywords.Password Confirmation') }}&ThinSpace;
                                        <b
                                            class="text-danger">({{ trans('keywords.leave empty if you dont want to change your password') }})</b></label>
                                ...
                                <div class="form-group col-12 col-md-6">
                                    ...
                                    <select name="role_name" class="form-control" id="role_name">
                                        @foreach ($roles as $role)
                                            <option @selected($admin->user->roles->first()->id == $role->id) value="{{ $role->name }}">
                                                {{ $role->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('role_name')
                                            ...
                                            <center>
                                                @if ($admin->photo)
                                                    <img class="upload-img-view" id="viewer"
                                                        src="{{ asset('storage/' . $admin->photo) }}" alt="image" />
                                                @else
                                                    <img class="upload-img-view" id="viewer"
